...
class ReceiptVouchers(Document):
    ...
    def gl_entry(self,account, amount, en_type, party_type, party, name):
        gl = frappe.get_doc(dict(doctype="GL Entry", posting_date=frappe.utils.nowdate(), transaction_date=frappe.utils.nowdate(), account=account, party_type=party_type, party=party, voucher_type="Receipt Vouchers", voucher_no=self.name,  remarks="No Remarks"))
        if(en_type == "credit"):
            gl.credit = amount
